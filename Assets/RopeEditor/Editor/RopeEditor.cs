...
public class RopeEditor : Editor
{
    ...
    private void DrawHinges(Rope rope)
    {
            ...
            rope.transform.childCount > 0)
        {
            ...
            if (GetConnectedObject(hingePositionInWorldSpace, firstSegment.GetComponent<Rigidbody>()))
        ...
        if (rope.WithPhysics && rope.HangLastSegment)
        {
            ...
            if (GetConnectedObject(hingePositionInWorldSpace, lastSegment.GetComponent<Rigidbody>()))
                GUI.color = Color.magenta;
            else
                GUI.color = Color.blue;
    ...
    static Rigidbody GetConnectedObject(Vector2 position, Rigidbody originalObj)
    {
        Rigidbody[] sceneRigidbodies = originalObj.GetComponentsInChildren<Rigidbody>();
    ...
    private static void UpdateEndsJoints(Rope rope)
    {
            ...
            rope.transform.childCount>0)
        {
            ...
            HingeJoint joint = firstSegment.gameObject.GetComponent<HingeJoint>();
            if(!joint)
                joint = firstSegment.gameObject.AddComponent<HingeJoint>();
            Vector2 hingePositionInWorldSpace = rope.transform.TransformPoint(rope.FirstSegmentConnectionAnchor);
            ...
            joint.connectedBody = GetConnectedObject(hingePositionInWorldSpace, firstSegment.GetComponent<Rigidbody>());
        ...
        if (rope.WithPhysics && rope.HangLastSegment)
        {
            HingeJoint[] joints = lastSegment.gameObject.GetComponents<HingeJoint>();
            HingeJoint joint = null;
            if (joints.Length > 1)
                joint = joints[1];
            else
                joint = lastSegment.gameObject.AddComponent<HingeJoint>();
            Vector2 hingePositionInWorldSpace = rope.transform.TransformPoint(rope.LastSegmentConnectionAnchor);
            joint.connectedAnchor = hingePositionInWorldSpace;
            joint.anchor = lastSegment.transform.InverseTransformPoint(hingePositionInWorldSpace) ;
            joint.connectedBody = GetConnectedObject(hingePositionInWorldSpace, lastSegment.GetComponent<Rigidbody>());
            if (joint.connectedBody)
            {
                joint.connectedAnchor = joint.connectedBody.transform.InverseTransformPoint(hingePositionInWorldSpace);
            }
    ...
    private static void AddJoint(Rope rope, float dtheta, float segmentHeight, Rigidbody previousSegment, GameObject segment)
    {
        ...
        joint.axis = rope.HingeAxis;

        if (rope.Use3DColliders)
        {
            BoxCollider boxCol = segment.AddComponent<BoxCollider>();
            boxCol.size = new Vector3(boxCol.size.x, boxCol.size.y, 1);
        }
