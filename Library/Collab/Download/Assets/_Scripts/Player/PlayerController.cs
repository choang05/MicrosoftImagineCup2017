...
public class PlayerController : MonoBehaviour
{
    ...
    public static int xVelocityHash = Animator.StringToHash("xVelocity");
    ...
    void Update()
    {
        ...
        if (isControllable)
        {
            //if (Input.GetButtonUp("Interact") && charController.isGrounded)
    ...
    private void AlignWithGroundNormal()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (charController.isGrounded && Mathf.Abs(Vector3.Angle(Vector3.up, hit.normal)) < 30)
            {
